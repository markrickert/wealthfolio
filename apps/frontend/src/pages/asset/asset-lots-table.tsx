    ...
    >
      ...
      <TableCell className="text-right tabular-nums">
        ...
        {!isSnapshot && lot.splitRatio !== 1 && (
          <div className="text-muted-foreground text-[11px]">
            {t("asset:lots.adj")}{" "}
            <PrivacyAmount value={lot.unitCost / lot.splitRatio} currency={currency} />
          </div>
        )}
      </TableCell>
      <TableCell className="text-right tabular-nums">
    ...
    <div className={cn("space-y-2 p-4", lot.isClosed && "opacity-60")}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 space-y-0.5">
          <div className="text-sm font-medium">{formatLotDate(lot, t)}</div>
          <div className="text-muted-foreground text-[11px]">
            {isSnapshot
              ? t("asset:lots.as_of_snapshot")
              : t("asset:lots.held", { period: formatHoldingPeriod(lot.acquisitionDate, t) })}
...
function formatLotDate(lot: AssetLotView, _t: TFunction) {
